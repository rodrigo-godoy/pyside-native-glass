...
class NativeGlassWidget(QWidget):
    def __init__(self, style=GlassStyle.SIDEBAR, parent=None):
        ...
        self._style = style
        self._border_radius = 0 # Default 0 para llenar layout
        ...
        if sys.platform == "darwin":
            # macOS: Modo Nativo Full (Soporta anidación)
            ...
            # Windows: Modo Híbrido Seguro
            # CRÍTICO: NO forzar WA_NativeWindow aquí. 
            # Si el widget es huérfano y luego se vuelve hijo, Qt manejará el cambio a Alien Widget.
            # Esto evita el conflicto de doble DWM.
            
            self.setAttribute(Qt.WA_NoSystemBackground, True)
            # self.setAttribute(Qt.WA_NativeWindow, True) <--- ELIMINADO INTENCIONALMENTE
    ...
    def showEvent(self, event):
        ...
        # Aplicar efecto SOLO si es ventana TopLevel real (Main Window)
        # Si es un hijo (Sidebar), NO toca el DWM.
    ...
    def paintEvent(self, event):
        if sys.platform == "win32":
            if self.isWindow():
                # Ventana Principal: Limpiar para que se vea el Acrylic nativo
                painter = QPainter(self)
                painter.setCompositionMode(QPainter.CompositionMode_Clear)
                painter.fillRect(self.rect(), Qt.transparent)
            else:
                # Panel Hijo (Sidebar/Content): SIMULACIÓN
                # Pinta un tinte sobre el fondo borroso de la ventana madre
    ...
    def _paint_windows_simulation(self):
        ...
        # --- MATERIALES WINDOWS (ALPHAS AJUSTADOS) ---
        # Sidebar: Más oscuro/denso (Alpha 200 = ~80%)
        if self._style == GlassStyle.SIDEBAR:
            color = QColor(25, 25, 25, 200) if is_dark else QColor(245, 245, 245, 200)
        
        # Content/Popover: Mucho más transparente (Alpha 50 = ~20%)
        # Esto hace que el desenfoque del fondo se note mucho más
...
def apply_glass_logic(target_object, style, mode):
    ...
    if sys.platform == "darwin":
        ...
        bg = "000000" if use_dark else "FFFFFF"
        # Alpha base para la ventana principal. 
        # CC (204) es un buen balance para que el Acrylic se note pero el texto se lea.
        alpha = "CC" 
